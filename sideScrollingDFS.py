...
def makeLava(app):
    app.lavaRow -= 1
    for col in range(app.cols):
        app.board[app.lavaRow][col] = 'cyan'
...
def timerFired(app):
    ...
    if isCollision(app):
        app.gameOver = True
        return
    if app.board[app.playerRow][app.playerCol] == 'cyan':
        app.gameOver = True
        return
...
def sideScroll(app):
    ...
    # bug here: going down doesn't keep track of the previous maze that goes
    # beneath the board
    for mazeCell in app.mazeBlockCells:
        (row, col) = mazeCell
        if row >= app.rows:
    ...
    # for mazeCell in app.mazeBlockCells:
    #     (row, col) = mazeCell
    #     app.newMazeBlockCells.append((row + app.scrollY, col))
    #     if row + app.scrollY >= app.rows or row + app.scrollY < 0:
    #         continue
    #     app.board[row][col] = app.mazeColor
...
def redrawAll(app, canvas):
    ...
    if app.gameOver == True:
        canvas.create_text(app.width // 2, app.height // 2 - app.cellSize, text='Game Over!',
        fill = 'white')
